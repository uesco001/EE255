...
//		printk("R%d / T%d = %d",R_old,Ttime[i],sum);
	}
	R_new += C_low;
...
SYSCALL_DEFINE4(set_rtmon,pid_t, pid, struct timespec *,C, struct timespec *,T, int,cpuid)
{
	...
	unsigned int C_array[30][4];
	unsigned int T_array[30][4];
	unsigned int size[4];

	unsigned long CC;  // store C time
	unsigned long TT; // store T time

	unsigned int C_low[4];
	unsigned int T_low[4];
	int cpu_id;
...
// being searching throuhg proceess and threads	
	...
	for_each_process_thread(p,t){
			...
			if(CC != 0 && TT != 0){
				...
				C_array[size[cpu_id]][cpu_id] = CC;
				T_array[size[cpu_id]][cpu_id] = TT;
				size[cpu_id]++;
		        	 printk("PID %d, C %lu ms, T %lu ms, cpuid %d \n",t->pid,CC,TT,t->cpu_i);	
...
// end the search of process and threads
	...
	for( j = 0; j < 4; j++)
	{
	 for(w = 0;w < size[j]; w++)
	 {
	  CPUX[j] += (C_array[w][j] * 100 )/ T_array[w][j]; 
         }
		printk("CPU%d utility at %d\n",j, CPUX[j]);	
	}
	goto skip;
...
///  staring task search
	...
	task->cpu_i = cpuid;
	if(task->cpu_i != -1){
		printk("stored cpu id as %d\n",task->cpu_i);
	}else{ printk("cpu id not set\n");}
